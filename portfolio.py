#If necessaty: pip3 install matplotlib
import numpy as np
...
#required returns (better to be input from customers)
rq_returns = [0.002,0.004]

#risk-free rate (better to be retrieved from internet)
rf = 0

port_price = read_prices("ASSET.csv")
returns = get_return(port_price)

#Possible Portfolios
port_risks, port_returns = possible_portfolios(returns,1000)
ef_risks, ef_returns, ef_weights = efficient_frontier(returns)
max_sp, tangency_index = sharpe_ratio(ef_weights,returns,rf)

#Outputs

#Tangency Portfolio Weights
print(ef_weights[tangency_index])
...
#risk-free: risk and return
#tangency portfolio:risk and return
